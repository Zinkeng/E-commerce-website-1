...
function Cart({ cartItems, onAdd, onRemove, onDelete }) {
  ...
  return (
    <div>
      ...
      {cartItems.length === 0 && (
        <div style={{ marginTop: "90px" }}>Cart is empty</div>
      )}
      {cartItems.map((item) => (
        <div class="card" key={item.id} style={{ width: "100vw" }}>
          <div class="row border-top border-bottom">
            <div class="row main align-items-center">
              <div class="col-2">
                <img
                  class="img-fluid rounded-start height-auto pt-5 cartImage"
                  src={item.image}
                  alt=""
                />
              </div>
              ...
              <div class="col-2">
                <button className="qtyBtn" onClick={() => onRemove(item)}>
                  -
                </button>
                {item.qty}
                <button className="qtyBtn" onClick={() => onAdd(item)}>
                  +
                </button>
              </div>
              <div class="col">
                {item.qty} x {item.price.toLocaleString("en-US")} FCFA
              </div>
              <div className="col ">
                <button className="qtyBtn" onClick={() => onDelete(item)}>
                  x
                </button>
              </div>
            </div>
